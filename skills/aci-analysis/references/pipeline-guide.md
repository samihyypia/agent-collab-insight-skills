...
- 本工具设计用于分析用户本人的对话日志。分析他人数据前，必须获得数据所有者的明确授权。
- `.aci` 产物可能包含敏感信息（业务逻辑、代码片段、文件路径等），共享前请确认已脱敏。
- 未经授权分析他人对话数据可能违反《个人信息保护法》《数据安全法》《网络安全法》等相关法律，由使用者承担全部责任。
- 将分析产物交给境外 AI 服务（如 Codex、Claude）时，注意数据跨境传输的合规要求。
